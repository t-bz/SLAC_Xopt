...
class MinimizeEmitBmag(ScipyMinimizeEmittanceXY):
    ...
    scipy_options: dict = Field(None, description="options to pass to scipy minimize")
    thick_quad: bool = Field(
        True,
    ...
    def get_execution_paths(
        self, model: ModelList, bounds: Tensor, tkwargs=None, verbose=False
    ):
        ...
        if (self.x_key and self.rmat_x is None) or (self.y_key and self.rmat_y is None):
            raise ValueError(
                "must provide rmat for each transverse dimension (x/y) being modeled."
            )
        ...
        if self.method == "Grid":
            ...
            linspace_list = [
                torch.linspace(
                    tuning_bounds.T[i][0] + eps,
                    tuning_bounds.T[i][1] - eps,
                    self.n_grid_points,
                )
                for i in range(dim)
            ]
            ...
            f_values, emit, bmag, is_valid, validity_rate, bss = self.evaluate_target(
                model,
                mesh_pts,
                bounds,
                tkwargs=tkwargs,
            ...
            best_x = torch.index_select(mesh_pts, dim=0, index=best_id).reshape(
                self.n_samples, 1, -1
            )
            ...
            for sample_id in range(self.n_samples):
                ys_exe = torch.cat(
                    (
                        ys_exe,
                        torch.index_select(
                            bss[sample_id], dim=0, index=best_id[sample_id]
                        ),
                    ),
                    dim=0,
                )
                emit_best = torch.cat(
                    (
                        emit_best,
                        torch.index_select(
                            f_values[sample_id], dim=0, index=best_id[sample_id]
                        ),
                    ),
                    dim=0,
                )
            ...
            if self.init == "smallest":
                if len(self.observable_names_ordered) == 1:
                    bss = model.models[0].outcome_transform.untransform(
                        model.models[0].train_targets
                    )[0]
                if len(self.observable_names_ordered) == 2:
                    bss_x, bss_y = [
                        m.outcome_transform.untransform(m.train_targets)[0]
                        for m in model.models
                    ]
                    bss = torch.sqrt(bss_x * bss_y)

                x_smallest_observed_beamsize = (
                    model.models[0]
                    ._original_train_inputs[torch.argmin(bss)]
                    .reshape(1, -1)
                )
                ...
                x_tuning_best = torch.index_select(
                    x_smallest_observed_beamsize, dim=1, index=tuning_dims
                )
                # x_tuning_init = x_tuning_best.repeat(self.n_samples,1).flatten()
                x_tuning_random = unif_random_sample_domain(
                    self.n_samples, tuning_domain
                ).double()
                x_tuning_init = (
                    (1.0 - self.jitter) * x_tuning_best.repeat(self.n_samples, 1)
                    + self.jitter * x_tuning_random
                ).flatten()
            if self.init == "best":
                tuning_dims = list(range(bounds.shape[1]))
                tuning_dims.remove(self.meas_dim)
                tuning_dims = torch.tensor(tuning_dims)
                x_data = model.models[0].input_transform.untransform(
                    model.models[0].train_inputs[0]
                )  # shape n_tuning x ndim
                x_tuning = torch.index_select(
                    x_data, dim=1, index=tuning_dims
                )  # shape n_tuning x n_tuning_dims
                # x_tuning = torch.cat((x_data[:,:self.meas_dim], x_data[:,temp_id:]), dim=1) #shape n_tuning x n_tuning_dims
                emit_init = self.evaluate_posterior_emittance_samples(
                    model, x_tuning, bounds, n_samples=10
                )[
                    0
                ]  # shape n_samples x n_tuning
                ...
                x_tuning_init = (
                    (1.0 - self.jitter)
                    * x_tuning[best_tuning_id].reshape(1, -1).repeat(self.n_samples, 1)
                    + self.jitter * x_tuning_random
                ).flatten()
            ...
            def target_func_for_torch(x_tuning_flat):
                return self.sum_samplewise_emittance_target(
                    sample_funcs_list, x_tuning_flat, bounds, cpu_tkwargs
                )
            ...
            bounds_for_scipy = (
                tuning_domain.repeat(self.n_samples, 1).detach().cpu().numpy()
            )
            ...
            emit_best, is_valid = self.evaluate_posterior_emittance_samples(
                sample_funcs_list,
                x_tuning_best,
                bounds,
                tkwargs=cpu_tkwargs,
            ...
            x_tuning_best_retained = torch.index_select(
                x_tuning_best, dim=0, index=cut_ids
            )
    ...
    def sum_samplewise_emittance_target(
        self, sample_funcs_list, x_tuning_flat, bounds, tkwargs
    ):
        # assert len(x_tuning_flat.shape) == 1 and len(x_tuning_flat) == self.n_samples * (bounds.shape[1]-1)
        assert len(x_tuning_flat.shape) == 1 and len(x_tuning_flat) == 1 * (
            bounds.shape[1] - 1
        )

        x_tuning = x_tuning_flat.double().reshape(1, 1, -1)

        sample_emittance = self.evaluate_posterior_emittance_samples(
            sample_funcs_list,
            x_tuning,
            bounds,
            tkwargs,
    ...
    def evaluate_target(
        ...
        (
            ...
            validity_rate,
            bss,
        ) = self.evaluate_posterior_emittance_samples(
            model, x_tuning, bounds, tkwargs, n_samples, transform_target
        )
    ...
    def evaluate_posterior_emittance_samples(
        self,
        model,
        x_tuning,
        bounds,
        ...
        if isinstance(model, ModelList):
            ...
            )  # result shape n_samples x n_tuning_configs*n_steps x num_outputs (1 or 2)

            x = x.reshape(
                x_tuning.shape[0], self.n_steps_measurement_param, -1
            )  # result n_tuning_configs x n_steps x ndim
            x = x.repeat(n_samples, 1, 1, 1)
            # result shape n_samples x n_tuning_configs x n_steps x ndim
            bss = bss.reshape(
                n_samples, x_tuning.shape[0], self.n_steps_measurement_param, -1
            )
            ...
            beamsize_squared_list = [
                sample_funcs(x).reshape(
                    *x_tuning.shape[:-1], self.n_steps_measurement_param
                )
        ...
        if self.x_key and not self.y_key:
            k = (
                x[..., self.meas_dim] * self.scale_factor
            )  # n_samples x n_tuning x n_steps
        ...
        emit, bmag, sig, is_valid = compute_emit_bmag(
            k, beamsize_squared, self.q_len, rmat, beta0, alpha0, thick=self.thick_quad
        )
        ...
        if transform_target:
            bmag_squared = torch.nan_to_num(bmag, 1.0).pow(2)
            emit_squared = sig[..., 0, 0] * sig[..., 2, 0] - sig[..., 1, 0] ** 2
            # result shape (n_samples x n_tuning) or (2*n_samples x n_tuning)
            if self.x_key and self.y_key:
                res = (
                    bmag_squared[: bss.shape[0]].pow(2)
                    * emit_squared[: bss.shape[0]].pow(2)
                    * bmag_squared[bss.shape[0] :].pow(2)
                    * emit_squared[bss.shape[0] :].pow(2)
                ).sqrt()
                is_valid = torch.logical_and(
                    is_valid[: bss.shape[0]], is_valid[bss.shape[0] :]
                )
            else:
...
def plot_virtual_emittance(
    ...
    if len(optimizer.generator.algorithm.observable_names_ordered) == 2:
        if dim == "x":
            ...
            bax_model_ids = [
                optimizer.generator.vocs.output_names.index(algorithm.x_key)
            ]
        elif dim == "y":
            algorithm = copy.deepcopy(optimizer.generator.algorithm)
            algorithm.x_key = None
            bax_model_ids = [
                optimizer.generator.vocs.output_names.index(algorithm.y_key)
            ]
        else:
            algorithm = copy.deepcopy(optimizer.generator.algorithm)
            bax_model_ids = [
                optimizer.generator.vocs.output_names.index(name)
    ...
    for i in range(n_tuning_dims):
        ...
        x_tuning[:, i] = x_scan
        target, emit, bmag, is_valid, validity_rate, bss = algorithm.evaluate_target(
            bax_model,
            x_tuning,
            bounds,
            tkwargs,
        ...
        for j in range(len(x_scan)):
            cut_ids = torch.tensor(range(len(target[:, j])), device=tkwargs["device"])[
                is_valid[:, j]
            ]
            target_valid = torch.index_select(target[:, j], dim=0, index=cut_ids)
            q = torch.tensor([(1.0 - ci) / 2.0, 0.5, (1.0 + ci) / 2.0], **tkwargs)
            if len(cut_ids) >= 10:
                quant = torch.quantile(target_valid, q=q, dim=0).reshape(1, -1)
            else:
                quant = torch.tensor(
                    [[float("nan"), float("nan"), float("nan")]], **tkwargs
                )
