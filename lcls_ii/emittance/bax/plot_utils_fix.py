...
def plot_virtual_emittance_vs_tuning(
    optimizer, x_origin, ci=0.95, tkwargs: dict = None, n_samples=10000
):
    """
    ...
    for i in range(n_tuning_dims):
        ...
        x_tuning[:, i] = x_scan
        emit, is_valid, validity_rate = algorithm.evaluate_posterior_emittance_samples(
            bax_model, x_tuning, bounds, tkwargs, n_samples, transform_target=False
        )
        quants = torch.tensor([])

        for j in range(len(x_scan)):
            cut_ids = torch.tensor(range(len(emit[:, j])), device=tkwargs["device"])[
                is_valid[:, j]
            ]
